...
const classNamesBind = require('classnames/bind');
import styles from 'flexboxgrid';
import formStyle from './Forms.styl';
const cx = classNamesBind.bind(styles, formStyle);
...
var Row = React.createClass({
  ...
  renderLabel: function() {
    ...
    labelClassNames.push(cx('control-label'));

    if (this.props.layout === 'horizontal') {
      labelClassNames.push(cx('col-sm-3'));
    }
  ...
  render: function() {
    ...
    if (this.props.layout === 'horizontal') {
      ...
      cssClasses.elementWrapper.push(cx('col-sm-9'));
    ...
    return (
        <div className={cx(cssClasses.row)}>
            {this.renderLabel()}
            {element}
        </div>
    );
